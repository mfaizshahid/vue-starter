...
<template>
    <!-- Navbar-->
    <v-app-bar :elevation="0" :height="120" app border="b" class="navbar" color="background" flat>

        <section class="d-flex ga-4 w-100">
            <!-- Route name -->
            <div class="base-card flex-grow-1">
                <div class="d-flex align-center ga-2">
                    <v-app-bar-nav-icon @click.stop="appStore.setSidebarDrawer(!sidebarDrawer)"></v-app-bar-nav-icon>
                    ...
                    <div v-else>
                        ...
                        }" color="primary">{{ $route.meta.activeIcon }}
                        </v-icon>
                    </div>
            ...
            <div class="base-card cursor-pointer">
                <div class="d-flex">
                    ...
                    <h3 v-if="authStore.user">{{ authStore.user.firstName }}</h3>
                </div>

                <v-menu activator="parent">
                    <v-list density="compact" nav>
                        <v-list-item append-icon="mdi-account" link title="Profile"
                            :to="AppRoutes.User.SETTINGS.PROFILE" />

                        <v-list-item @click="logoutUser" append-icon="mdi-logout" link title="Logout" />
                    </v-list>
                </v-menu>
            </div>
        </section>
